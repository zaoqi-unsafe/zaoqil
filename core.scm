...
(struct rest (x r) #:transparent)
(struct readsymbol (cs) #:transparent)
(define (readc xs)
  (or (read-comment xs)
      (read-boolean xs)
      (read-list xs)
...
(define (read-boolean xs)
  (and (eq? (car xs) #\#)
       (let ([nxs (cdr xs)])
         (or (and (eq? (car nxs) #\t) (rest #t (cdr nxs)))
             (and (eq? (car nxs) #\f) (rest #f (cdr nxs)))))))
(define (read-list xs)
  (and (eq? (car xs) #\()
       (%read-list (cdr xs))))
(define (%read-list xs)
  (if (eq? (car xs) #\))
      (rest '() (cdr xs))
      (let ([x (readc xs)])
        (and x (let ([rs (%read-list (rest-r x))])
                 (and rs
                      (rest (cons (rest-x x) (rest-x rs)) (rest-r rs))))))))
(define (read-char xs)
  (and (eq? (car xs) #\#)
       (let ([nxs (cdr xs)])
         (and (eq? (car xs) #\\)
              (let ([nnxs (cdr nxs)])
                (rest (car nnxs) (cdr nnxs)))))))
(define (read-quote xs)
  (and (eq? (car xs) #\')
       (let ([x (readc (cdr xs))])
         (and x (rest (list 'quote (rest-x x)) (rest-r x))))))
(define (space? x)
  ...
  (and
   ...
   (not (delimiters? (car xs)))
       (let ([n (read-symbol (cdr xs))])
         (if n
             (rest (readsymbol (cons (car xs) (readsymbol-cs (rest-x n)))) (rest-r n))
             (rest (readsymbol (list (car xs))) (cdr xs))))))
